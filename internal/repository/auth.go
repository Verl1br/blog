...
func (r *AuthorizationRepository) CreateUser(user model.User) (int, error) {
	...
	query := fmt.Sprintf("INSERT INTO %s (first_name, last_name, email, password_hash) VALUES ($1, $2, $3, $4) RETURNING id", "users")
...
func (r *AuthorizationRepository) GetUser(email string) (model.User, error) {
	...
	query := fmt.Sprintf("SELECT id, password_hash FROM %s WHERE email = $1", "users")
